...
def send_button(chat_id, message, keyboard):
    vk.messages.send(chat_id=chat_id,
                     message=message,
                     random_id=random.randint(0, 2 ** 64),
...
def add_buttons_play_and_rules(chat_id):
    keyboard = VkKeyboard(one_time=True)
...
Напомнить букву: hint (напомнить букву)
                ...
                if 'rules' in text or 'правила' in text:
                    send_message(chat_id, '''Предлагаем сыграть в нашу игру под названием «Угадай город».
...
Вы не можете писать города, которые уже раннее были назаваны.''')
                ...
                if 'end' in text or 'закончить игру' in text:
                    send_message(chat_id, f'Количество названных вами городов: {counter_towns}')
                    game_started = False
                ...
                if game_started and 'hint' not in text and \
                    ...
                    if not check_town_in_selected_list(town):
                        send_message(chat_id, 'Такой город уже был >:(')
                        send_message(chat_id, f'Назовите город на букву "{last_letter_bot}"')
                    elif not check_town_in_list(town):
                        send_message(chat_id, 'Такого города не существует >:(')
                        send_message(chat_id, f'Назовите город на букву "{last_letter_bot}"')
                        ...
                        send_message(chat_id, town_bot)
                        counter_towns += 1
                    else:
                        send_message(chat_id, 'Город не подходит условию')
                        send_message(chat_id, f'Назовите город на букву "{last_letter_bot}"')
                ...
                if 'hint' in text or 'напомнить букву' in text:
                    send_message(chat_id, f'Вы должны назвать город на букву "{last_letter_bot}"')
                ...
                if game_started:
                    add_buttons_end_and_hint(chat_id)
                # иначе бот отправляет саму кнопку игры и как в нее играть
                else:
                    add_buttons_play_and_rules(chat_id)
